...
(async () => {
  ...
  const expression = [
    ...
    '    for (let i = 0; i < 240; i++) { g.player.yardAngle = g.player.idealYard; g._fixedUpdate(1/60); }',
    '    out[label] = { eff: Math.round(g.player.sailEfficiency*100), knots: Number(g.player.speedKnots.toFixed(1)), grounded: g.player.grounded };',
    '  }',
    '  const wf = g.windAngle;',
    '  await run("downwind", wf + Math.PI);',
    '  await run("beam", wf + Math.PI/2);',
    '  await run("headwind", wf);',
    '  g.player.resetState(0, 40, wf + Math.PI);',
    '  g.player.sailAmount = 1;',
    '  for (let i=0;i<240;i++){ g.player.yardAngle=g.player.idealYard; g._fixedUpdate(1/60); }',
    '  out.sailKnots = Number(g.player.speedKnots.toFixed(1));',
    '  g.player.toggleAnchor();',
    '  for (let i=0;i<240;i++) g._fixedUpdate(1/60);',
    '  out.anchorKnots = Number(g.player.speedKnots.toFixed(2));',
    '  // ????????',
    '  g.player.toggleAnchor();',
    '  const enemy = g.enemies.enemies.find(e=>e.ship.alive);',
    '  const hp0 = enemy.ship.hp;',
    '  let sunk=false;',
    '  for(let i=0;i<6;i++){ if(enemy.ship.damage(14,"cannonball")){sunk=true;break;} }',
    '  out.damage = hp0 - enemy.ship.hp;',
    '  out.sunk = sunk;',
    '  for(let i=0;i<220;i++) g.enemies.update(1/60,g.player,g.ocean,g.world,{spyglass:false});',
    '  out.enemyRemoved = !enemy.ship.alive;',
    '  // AI ???',
    '  const other = g.enemies.enemies.find(e=>e.ship.alive);',
    '  if(other){ other.ship.position.set(0,0,-60); g.player.position.set(0,0,-120);',
    '    for(let i=0;i<260;i++) g.enemies.update(1/60,g.player,g.ocean,g.world,{spyglass:false});',
    '    out.aiState = other.state; }',
    '  // ????',
    '  g.save();',
    '  const raw = JSON.parse(localStorage.getItem("endless-archipelago-save-v1"));',
    '  out.saveOk = typeof raw.seed==="number" && !!raw.player;',
    '  // ?????',
    '  g.player.resetState(0,40,wf+Math.PI); g.player.sailAmount=1;',
    '  const counts=new Set();',
    '  for(let i=0;i<3000;i++){ g._fixedUpdate(1/60); if(i%300===0) counts.add(g.world.chunkCount); }',
